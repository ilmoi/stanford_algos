...
# Kosaraju
def kosaraju(edges):
    # OLD SUBROUTINE - BREAKS AFTER NUM OF RECURSIVE CALLS EXCEEDED
    # def internal_DFS(edges, s):
    #     nonlocal explored
    #     nonlocal Ts
    #     nonlocal t
    #     nonlocal counter
    #
    #     explored[s] = True  # set item O1
    #     leaders[s] = curr_leader  # set item O1
    #     # print(f">> NODE is {s}")
    #     for e in edges:  # O(m)
    #         if e[0] == s:  # O1
    #             v = e[1]  # O1
    #             # print(f"following edge {e} from {s} to {v}")
    #             if not explored[v]:  # get item O1
    #                 # print(f'adding {v}...')
    #                 counter += 1
    #                 print(f'counter is {counter}')
    #                 internal_DFS(edges, v)  # O n... hm so right now it's O n*m?
    #             # else:
    #                 # print(f'{v} already explored')
    #     Ts[s] = t
    #     t += 1
    ...
    def better_interanl_DFS(adj_nodes, s):
        ...
        # have to define internally because of nonlocals + specificity around T and leader
        nonlocal explored
        nonlocal Ts
        nonlocal t
        nonlocal counter
    ...
    # get node number
    R_adj_nodes = make_adj_nodes(Redges)
    n = len(R_adj_nodes)
